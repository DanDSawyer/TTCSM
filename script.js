...
{
	g__shareInput.value = `tcmperks.com?char=${charName}&build=${tcm__encodeBuildString(charName)}`;
}
...
{
	let iconElem;
	let perkObject = tcm__getPerkObject(perkId);
	//not empty
	if (perkObject != undefined)
	{
		...
		{
			if (perkObject[0] < 200) /* victim */
			{
				iconElem.classList.add('victim');
			}
			else if (perkObject[0] < 400) /* family */
			{
				iconElem.classList.add('family');
				columns = g__familyPerksColumns;
				rows = g__familyPerksRows;
			}
			else if (perkObject[0] < 600) /* ability */
			{
			...
			iconElem.classList.add(perkObject[2]);
			iconElem.setAttribute('data-id', perkObject[0]);
		}
		iconElem.style.width = `${iconSize}px`;
		iconElem.style.height = `${iconSize}px`;
		iconElem.style.backgroundPosition = `${(perkObject[4][0])*(100/(columns-1))}% ${(perkObject[4][1])*(100/(rows-1))}%`;
